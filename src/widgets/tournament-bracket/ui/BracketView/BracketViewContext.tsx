...
interface BracketViewContextValue {
  ...
  theme: ThemeConfig // NEW: Theme is now safely typed in context
}

const BracketViewContext = createContext<BracketViewContextValue | null>(null)

export function BracketViewProvider({ children, ...value }: React.PropsWithChildren<BracketViewContextValue>) {
  return <BracketViewContext.Provider value={value}>{children}</BracketViewContext.Provider>
}
